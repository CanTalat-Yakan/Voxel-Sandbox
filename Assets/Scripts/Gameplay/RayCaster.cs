...
public class RayCaster : Component
{
    ...
    public void Initialize(GameManager gameManager, Entity cube, Entity camera)
    {
    ...
    public override void OnUpdate()
    {
        ...
        if (TargetVoxelPosition is not null)
            if (Input.GetButton(MouseButton.Left, InputState.Down))
                _gameManager.Generator.SetVoxel(_gameManager.NoiseSampler, TargetVoxelPosition.Value, VoxelType.None);

        if (AdjacentVoxelPosition is not null)
            if (Input.GetButton(MouseButton.Right, InputState.Down))
                _gameManager.Generator.SetVoxel(_gameManager.NoiseSampler, AdjacentVoxelPosition.Value, VoxelType.Sand);
    }
